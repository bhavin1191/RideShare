...
public class NewRideDaoImpl extends ModifiableCommonDBDao<NewRide, NewRide> implements NewRideDao {
	...
	@Override
	public List<RideAvailableForSharing> searchRideOnSource(SerializableLatLng source, SerializableLatLng destination) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (object != null) {
				RideAvailableForSharing ride = new RideAvailableForSharing((String) object[1], (int)object[5],
						String.valueOf(((BigInteger)object[0]).intValue()), (String) object[2], (String) object[3]);
				rides.add(ride);
